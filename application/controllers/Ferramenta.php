...
class Ferramenta extends CI_Controller
{
    ...
    public function add()
    {
        if ($this->input->post()) {
            ...
            if ($this->upload->do_upload('img')) {
                ...
                $img = '/assets/images/ferramentas/1.png';
            }
    ...
    function view($ferramenta_id)
    {
        $data['ferramenta'] = $this->Ferramenta_model->get_ferramenta($ferramenta_id);
        $data['armazenamentos'] = $this->Armazenamento_model->get_all_armazenamentos();
        $data['logs'] = $this->Log_model->get_logs_by_ferramenta($ferramenta_id);
    ...
    public function atualizar_armazenamento()
    {
        ...
        $ferramenta = $this->Ferramenta_model->get_ferramenta($ferramenta_id);
        if (!$ferramenta) {
            show_error('Ferramenta não encontrada.');
        }

        $this->Ferramenta_model->update_ferramenta($ferramenta_id, ['armazenamento' => $armazenamento]);
        $acao = $armazenamento == 0 ? 'retirado' : 'colocado';
        $this->log_ferramenta($usuario_id, $ferramenta_id, $ferramenta['calibragem'], $armazenamento, $acao);

        echo json_encode(['status' => 'success']);
    }

    private function log_ferramenta($usuario_id, $ferramenta_id, $calibragem, $armazenamento_id, $acao) {
        $log_data = array(
